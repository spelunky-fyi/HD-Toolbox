    ...
    <Cell span={12}>
      <div style="margin-top: -15px;">
        {#if $activeMethod == LoadMethod.LATEST}
          <i>
            Always use the most recent version of Frozlunky. You can hit refresh
            to check if new versions are available.
          </i>
        {:else if $activeMethod == LoadMethod.SPECIFIC_VERSION}
          <i>
            Choose a specific version of Frozlunky. You can hit refresh to check
            if new versions are available.
          </i>
        {:else if $activeMethod == LoadMethod.FILE}
          <i>
            Browse to a local exe. This is useful for development or if you
